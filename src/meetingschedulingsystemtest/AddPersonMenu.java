...
public class AddPersonMenu extends javax.swing.JFrame {
    ...
    private void submitPersonMenuButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_submitPersonMenuButtonActionPerformed
        ...
        String fullName = getFirstName().toUpperCase() + " " + getLastName().toUpperCase() + " " + getPhoneNumber();
        if(!validFirstName || ! validLastName || !validPhoneNumber || firstNumber.equals("0")){
            ...
            return;
             ...
             //validates the person and makes sure they don't already exist
            for(int j=0;j<MSSMM.personArray.size();j++){
                String fName = MSSMM.personArray.get(j).getFirstName();
                String lName = MSSMM.personArray.get(j).getLastName();
                String pNumber = MSSMM.personArray.get(j).getPhoneNumber();
                String fullPersonName = fName.toUpperCase() + " " + lName.toUpperCase() + " " + pNumber;
                if(fullPersonName.equals(fullName)){
                    dispose();
                    AddPersonMenu addPersonMenu = new AddPersonMenu();
                    JOptionPane.showMessageDialog(null, "That person exists already!", "Error", JOptionPane.ERROR_MESSAGE);
                    addPersonMenu.setVisible(true);
                    return;
                }
            }
